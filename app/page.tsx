...
export default function Home() {
  ...
  const networkFee = (
    ...
    Number(amount) +
    walletCount * 0.0001 +
    (walletCount / BATCH_SIZE) * 0.0003 +
    0.01
  ).toFixed(6);
  const [isValidCAddress, setIsValidCAddress] = useState<boolean>(false);
  const minWalletCount = MIN_WALLET_COUNT; // min B wallet count
  const maxWalletCount = MAX_WALLET_COUNT; // max B wallet count
  ...
  const distributeSol = async (
    ...
    try {
      ...
      const transferAmount =
        totalAmount +
        distributionNum * 0.0001 +
        (distributionNum / BATCH_SIZE) * 0.0003 +
        0.01;
      ...
      const recipientKeypairs = Array(distributionNum)
        .fill(0)
      ...
      for (
        ...
        while (retries > 0) {
          ...
          if (
            lookupTable &&
            lookupTable.state.addresses.length === currentBatch.length
          ) {
            break;
          }
          await sleep(2000);
        ...
        const distributionInstructions: TransactionInstruction[] =
          currentBatch.map((kp, idx) => {
            const actualIdx = batchStart + idx;
            // Ensure we're calculating a valid integer amount of lamports
            const solAmount = Math.max(
              1,
              Math.floor(proportions[actualIdx] * LAMPORTS_PER_SOL)
            );
        ...
        console.log(1);
        const distributionTransaction = new VersionedTransaction(
          distributionMessageV0
        );
        distributionTransaction.sign([intermediaryWallet]);
        console.log(2);
        try {
          const distributionTxSig = await connection.sendRawTransaction(
            distributionTransaction.serialize()
          );
          console.log(
            `Distributed batch ${batchStart}-${batchEnd}:`,
            `https://solscan.io/tx/${distributionTxSig}?cluster=devnet`
          );
      ...
      for (
        ...
        const gatherTxSig = await connection.sendRawTransaction(
          gatherTransaction.serialize()
        );
        console.log(
          `Gathered batch ${batchStart}-${batchEnd}:`,
          `https://solscan.io/tx/${gatherTxSig}?cluster=devnet`
        );
  ...
  const createAndSendV0Tx = async (
    ...
    try {
      ...
      console.log(
        "LUT transaction successfully confirmed!",
        "\n",
              ...
              <div>
                ...
                <div className="relative h-[2px] group">
                  ...
                  <motion.div
                    ...
                    style={{
                      width: `${
                        ((walletCount - minWalletCount) /
                          (maxWalletCount - minWalletCount)) *
                        100
                      }%`,
                    }}
                  />
